...
#figure(
    image("image/driving test simulator.jpeg"),
    caption: [Driving Test Simulator @Sim],
    placement: auto,
...
#context {
    ...
    meander.reflow({
        import meander: *
        placed(top + right, fig1, dy: -12pt)
        container(height: remaining)
        
        pagebreak()
        
        placed(top + left, fig2)
        placed(top + right, dy: 17.5em, fig3)
        placed(top + left, dy: 37em, fig4)
        container()
        pagebreak()
        
        placed(top + right, fig5)
        placed(top + left, dy: 16em, fig6)
        placed(top + right, dy: 37em, fig7)
        container()
        pagebreak()
        
        placed(top+left, fig8)
        container()
        container()
        content[#tex]
    })
...
== Related Works
The research community has proposed various methods for validating autonomous driving functions using simulation and @VIL concepts, as well as a wide range of lateral control strategies for path and lane tracking. This section summarizes the most relevant works and explains how they relate to the present thesis, which implements a @LKA function in the CARLA simulator using an external Python-based controller and an RGB camera.\
*@VIL simulation*

Son et al. propose a PG-Based @VIL Simulation framework that combines a real vehicle with a virtual driving environment for system development and consistency validation @electronics11244073. Their system includes modules for virtual road generation, synchronization between real and virtual domains, a virtual traffic manager, and perception sensor modelling. They demonstrate that the behavior observed in the @VIL Simulation setup is consistent with physical vehicle tests over different speeds, road types and surrounding environments. This work shows how @VIL can improve reproducibility and safety when testing autonomous driving systems.

Cheng et al. provide a comprehensive survey on testbench-based Vehicle-in-the-Loop simulation testing for @AV:pls @survey_test_bench. They review existing @AV testing approaches and identify limitations of traditional methods. The paper then focuses on testbench-based @VIL architectures, principles and equipment, with particular emphasis on physical signal stimulation and realistic road condition simulation. The survey identifies research gaps between current testbench technologies and industrial @AV testing needs, and highlights @VIL as a promising tool for accurate and efficient validation.

Xiong Hu et al. present a @VIL simulation test framework for @AV:pls, where an autonomous driving system is evaluated using a virtual environment combined with real vehicle components @xiong_hu. Their work emphasizes the synchronization of vehicle dynamics and simulated scenarios and discusses how @VIL can be used to test autonomous driving functions safely and systematically.

These works clearly show that @VIL is an important methodology for validating autonomous driving systems. However, they mainly focus on general architectures, testbench setups, and system-level validation. They do not specifically address a software-only @VIL configuration in a driving simulator like CARLA, where a simulated vehicle is controlled by an external code using RGB camera images. Furthermore, the detailed design and evaluation of a single ADAS function such as @LKA—implemented and tested entirely in such a @VIL setup—remains less explored. This is where the present thesis contributes.\
*Lateral control for @AV:pls*
...
Artuñedo et al. present a comparative evaluation of state-of-the-art lateral control strategies for @AV:pls @ARTUNEDO2024100910. They propose a systematic tuning methodology and define a comprehensive set of performance metrics covering tracking accuracy, robustness and comfort. The controllers are evaluated in extensive simulations and real-vehicle tests over different trajectories and scenarios. Their results show how different control strategies behave under the same conditions and provide guidance for selecting suitable lateral controllers.

Dominguez et al. experimentally compare several classical lateral controllers—such as Pure Pursuit and Stanley—on an @AV:pls platform @7795743. They study the performance of these controllers in terms of tracking error and stability, using a combination of simulation and real-world experiments. Their work provides practical insight into the behavior of commonly used path-tracking controllers and their suitability for autonomous driving applications.

Snider’s technical report is a widely cited reference on automatic steering methods for autonomous automobile path tracking @Snider2009AutomaticSM. It describes the mathematical formulation and implementation details of several steering controllers, including Pure Pursuit and Stanley, and discusses their application to @AV:pls. This report is frequently used as a theoretical foundation when implementing classical lateral control laws.

These studies provide a solid theoretical and experimental basis for the lateral control part of @LKA. However, most of them either assume ideal path or lane information (for example, predefined waypoints or ground-truth lane geometry) or focus on real-vehicle experiments without an explicit simulation-based @VIL framework. They do not analyze in detail how classical lateral controllers perform when they receive lane information from a real-time camera-based perception pipeline inside a simulator.\
