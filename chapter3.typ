...
The kinematic bicycle model introduced in @ch2 is the simplest description of the planar motion of a four-wheeled vehicle. It collapses the front pair of wheels into a single equivalent wheel placed at the front axle, and similarly for the rear pair, and ignores tire slip. Under these assumptions the configuration of the vehicle is fully described by three quantities: the position $(x, y)$ of a reference point—conventionally taken at the center of the rear axle—and the yaw angle $psi$ that the body of the vehicle makes with the world $x$-axis. The configuration space is therefore $RR^2 times S^1$, and a useful sketch of its variables is given in @bicycle.
...
In the CARLA Python API, the steering input is not the physical steering angle $delta$ but a normalized "steer command" $delta_c in [-1, +1]$ that the simulator scales internally by a vehicle-specific maximum angle $delta_max$. The relationship $delta = K_"steer" delta_c$ with $K_"steer" approx delta_max$ is approximately linear in the regime of interest @kebbati. Plugging this relationship into the yaw-rate equation gives the relation between the actuator command and the resulting yaw rate:
...
Two practical issues must be addressed in any discrete realization of @eqt:eq_pid_disc. The first is _integral wind-up_: when the steering command saturates at the physical limit of the actuator, the integral term keeps growing as long as the lateral error does not change sign. Once the vehicle finally crosses the center of the lane, the controller has to "discharge" this accumulated integral before it can produce a counter-steering action, which leads to a long undershoot. The simplest remedy, used in all three architectures of the present work, is to clamp the integral to a symmetric interval $[-I_max, +I_max]$ at every step.
...
=== Pole Placement
A natural choice for the closed-loop denominator of a PID compensating a double integrator is a triple real pole at $s = -omega_n$, which gives a critically damped behavior with bandwidth $omega_n$. The closed-loop characteristic polynomial of $K_p + K_i / s + K_d s$ acting on $K_"lat" / s^2$ with $K_"lat" = v^2 K_"steer" / L$ is
...
$ L_d(v) = L_d^"min" + k_L v $
with a minimum $L_d^"min" = 4$ m that protects against pathological behavior at standstill, and a slope $k_L = 0.6$ s that produces $L_d approx 9$ m at the nominal cruise speed of $30$ km/h. The same kind of linear scheduling is used by the CARLA reference local planner @local_planner for its waypoint-popping criterion, with comparable numerical values.
...
The third architecture removes the dependence on the pre-recorded `.csv` reference path by reconstructing the center line of the lane at every tick from the RGB camera feed. The architecture is identical to the heading-error PI controller of @sec_heading from the controller block downward; the only modification is the way in which the look-ahead point is generated. A high-level view of the vision-based pipeline is given on the bottom row of @three_arch.

=== The Lane-Detection Network
Lane detection in road scenes has been a benchmark task in computer vision for over a decade, and the literature offers a variety of architectures with different trade-offs between accuracy, speed, and complexity. The taxonomy adopted in the recent comparative study of Lin et al. @lin2024lane covers semantic-segmentation networks (SCNN @scnn, RESA @resa), row-based classifiers (UFLD @ufld), anchor-based detectors (LaneATT @laneatt, ADNet @adnet), parametric-curve regressors (BezierLaneNet @beziernet) and the recent CLRNet @clrnet family that combines an anchor-based proposal mechanism with row-wise refinement. The implementation supports any model from this list through a pluggable registry stored in the `detection_models` dictionary; the registry, the model-loading helpers, and the inference pipeline used in this thesis are built on top of the open-source UnLaneDet framework @unlanedet, which provides a unified PyTorch implementation of the architectures listed above and from which the lane-detection backbone of the present @LKA controller has been adapted. For the experiments reported in this thesis, the CLRNet model trained on the TuSimple dataset has been used, because it offers the best trade-off between latency and lane-fitting accuracy in the comparative study cited above.

The detection network operates on a $1280 times 720$ frame extracted from the camera sensor and produces a list of lane lines, each represented as a sequence of pixel coordinates $(u_i, v_i)$. In the present formulation, the reference for the controller is not a single lane line but the _center line_ of the ego-lane, which has to be reconstructed from the detected boundaries. The reconstruction is performed by the helper function `get_centerline`: only the lane lines that reach the bottom $30%$ of the image are kept—so that adjacent lanes and far-away lines are filtered out—and the closest left and right boundaries to the image center are paired up. Their interpolated mid-points at a regular sampling along the image $v$-axis form the center line, which is returned ordered from the closest sample to the furthest.
...
The three architectures presented in this chapter solve the same lane-keeping problem with progressively richer information and progressively more sophisticated control structure. From a purely control-theoretic standpoint, they can be summarized as in @arch_summary.
