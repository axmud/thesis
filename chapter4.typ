...
= Simulation Results and Evaluation <ch4>

The previous chapter developed three lateral-control architectures for the @LKA function and motivated each design choice on control-theoretic grounds. The present chapter complements that theoretical discussion with the numerical evaluation of the three controllers in CARLA. All experiments are run on the same test route in Town06, with the same vehicle (`vehicle.nissan.patrol`), the same target speed of $30$ km/h, the same simulation rate $T_s = 0.05$ s, and the same set of PID gains obtained from the system-identification procedure of @sec_sysid. Under these conditions, any observed difference between the trajectories can be attributed to the structure of the error signal and to the corresponding controller rather than to incidental variations in the simulator.

The chapter is structured as follows. @sec_exp_setup describes the experimental protocol, the reference path and the metrics used to compare the controllers. @sec_carla_view presents a qualitative view of the test route as rendered inside the CARLA world. @sec_full_traj quantifies the tracking performance over the entire route. @sec_smooth_turn and @sec_lane_change then zoom in on two representative sections of the route—the long smooth curve and a sudden lane discontinuity—where the differences between the three architectures become most visible. @sec_discussion finally interprets the results in the light of the control-theoretic predictions of @ch3.

== Experimental Setup <sec_exp_setup>
The reference path was recorded once, in autopilot mode, by driving the ego vehicle through Town06 and logging the closest waypoint at every simulation tick, as described in @ch2. The resulting `carla_data.csv` file contains a strictly monotonic sampling of the center line along an approximately $L$-shaped route of about $850$ m. The route starts on a north–south urban street, executes a $90 degree$ left turn onto a long east–west arterial, and ends after a stretch of straight road that includes a step-like lane discontinuity—a feature that proves particularly informative when comparing the three controllers. The route was selected because it exercises both the steady-state behavior of the controllers, on the long straight section that follows the turn, and their transient behavior, on the corner itself and on the lane discontinuity.
...
#list(
    [The _@RMSE:lo _, @RMSE $e_y = sqrt((1/N) sum_(k=1)^N e_y[k]^2)$, which summarizes the average tracking accuracy over the run;],
    [The _maximum absolute cross-track error_, $max abs(e_y) = max_(k) abs(e_y[k])$, which captures the worst-case excursion from the center of the lane.],
)
...
Two observations can already be made from the in-simulator view. First, the trajectory stays well inside the lane throughout the manoeuvre—the rendered line never leaves the asphalt and remains close to the center of the lane on both the curved and the straight sections of the route. This is the minimum requirement for a @LKA and the most basic safety check. Second, the corner itself is the most demanding part of the route: it concentrates a large change in the reference heading into a short stretch of road and produces the largest transients in the cross-track error, as the next section will show.
...
The numerical metrics extracted from the right-hand panel are summarized in @tab_full_metrics. The heading-error PI achieves both the lowest @RMSE and the lowest maximum excursion of the three architectures, by a clear margin. The cross-track PID is intermediate in @RMSE but exhibits the worst maximum excursion of the three runs at the lane discontinuity. The vision-based PI has the highest @RMSE, mainly because of a persistent steady-state bias on the smooth curve that the next section will analyze in detail.
...
The heading-error PI (green) reacts differently. The look-ahead point is, by construction, several meters ahead of the vehicle; the controller therefore "sees" the discontinuity coming and starts steering toward the new lane before the discontinuity is reached. The cross-track error in @fig_traj_change shows a much smaller positive transient of approximately $+1$ m, followed by a small undershoot of $-1$ m as the look-ahead point crosses the step in the opposite direction. The transient is over within a single look-ahead time horizon, that is in less than $1$ s. The maximum-excursion metric on this controller is the smallest of the three at $1.19$ m, which is essentially this single peak.

The vision-based PI (red) reacts earliest of all three because the lane detector reconstructs the upcoming center line several meters ahead of the vehicle and the polynomial fit smooths the discontinuity into a gradual lateral shift. The cross-track error trace shows a large negative excursion that starts at $t approx 67$ s—well before the path-based controllers react—and reaches $approx -2$ m at $t approx 70$ s. This is _not_ a tracking failure: the vehicle is correctly following what the camera sees, and what the camera sees is a smooth lateral shift rather than a step. The error is large because the metric is computed against the path-based reference, which itself contains the step. In a real-world deployment, where the only reference is the camera image, this trajectory would actually be the smoothest and the most natural-feeling of the three.
...
The numerical results of this chapter both confirm and refine the control-theoretic predictions of @ch3. The reduction of one plant integrator from the cross-track to the heading-error formulation buys a measurable improvement in tracking accuracy: the heading-error PI achieves an @RMSE of $0.120$ m against the $0.205$ m of the cross-track PID, a factor of $1.7$ improvement, and a maximum excursion of $1.19$ m against $2.22$ m, a factor of $1.9$ improvement. Both numbers are consistent with the larger phase margin and the gentler gain schedule that @arch_summary attributed to the heading-error formulation.
